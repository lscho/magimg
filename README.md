...
AI 抠图页（`/cutout`）在左侧主导航中位于“图生图”下方，也可从文生图/图生图结果区工具栏或图片编辑页右键菜单进入，沿用与生成页一致的两栏布局。页面始终显示透明棋盘工作台；无图片时不切换提示页，可通过左侧导入按钮或直接拖入 PNG、JPEG、WebP。框选使用原生 Pointer Capture，绘制时实时跟随指针；多个选区并存，序号位于选框内左上角，并支持单独删除、撤销与重做。右侧结果缩略图在鼠标悬停或键盘聚焦时显示完整透明素材的放大预览。右侧不展示模型名称、档位或开关；仅当首次检测到资源不完整时显示一个资源包下载提示，SAM 2.1 Hiera Base+ 与 ViTMatte Small 通过同一操作补齐。两部分均安装后提示自动隐藏，后续进入页面不再重复显示。

桌面端会在完整抠图成功后把原图、选区和全部透明 PNG 保存到 `appDataDir/cutout-history/`，任务清单由 plugin-store 管理，最多保留最近 100 条。创作历史中的“AI 抠图”页签支持多选下载和删除；任务卡右键只提供恢复工作和保存全部素材。恢复工作后调整选区会清空旧结果，再次抠图会重新扣费并新增独立历史任务，不覆盖原记录。浏览器预览不读取、写入或模拟这些记录。

SAM 2.1 模型的 encoder、decoder 及两份 external-data 权重会逐文件流式写入本地，并按固定大小和 SHA-256 校验；不再下载或解压旧 SAM ZIP。推理使用 Tauri/Rust 原生 `ort 2.0.0-rc.10` 与 ONNX Runtime 1.22，不加载 `onnxruntime-web` 或 WASM：前端将图片直接缩放到 1024x1024，按 ImageNet 均值方差生成 NCHW 输入，再通过 Raw IPC 发送；Rust 从 `appDataDir/models/` 白名单路径加载并再次校验四个文件，encoder 产生三层 image embedding 后立即释放大会话，decoder 与 embedding 留存供同一任务的多个框选复用。框选提示通过 `input_boxes` 传入，取消操作会终止当前原生运行。
...
SAM 2.1 ONNX 资源使用 Apache-2.0 许可，下载地址固定到提交 `bab18593f44e652f04cf18b60b3690f60e8996b0`，总计 `108676041` 字节。客户端使用该提交中的 `vision_encoder_quantized.onnx`、`vision_encoder_quantized.onnx_data`、`prompt_encoder_mask_decoder_quantized.onnx` 和 `prompt_encoder_mask_decoder_quantized.onnx_data`，避免上游 `main` 内容变化导致模型与校验信息不一致。旧 ViT-H、ViT-L、ViT-B 和 MobileSAM 文件不会被主动删除，但客户端不再展示、下载或加载。
