...
图片压缩页（`/compress`）是桌面专属的免费本地能力，位于左侧导航第二组首项。页面支持多选图片、拖放图片和递归选择单个源文件夹；文件夹模式把源根目录替换为输出根目录并保留相对结构，例如 `源根/a/x.png` 输出到 `输出根/a/x.png`。待处理列表按可视区域加载缩略图，鼠标悬停或键盘聚焦可查看不裁切的放大预览。PNG、JPEG、WebP 均保持原格式，不缩放、不上传、不写历史，也不要求登录或扣除积分。PNG 使用 `oxipng 10.1.1` 的三级无损优化；JPEG 使用 `image 0.25.10` 解码和 `mozjpeg-rs 0.9.2` 编码；WebP 使用 `image` 与静态链接 libwebp 的 `webp 0.3.1`；`img-parts 0.4.0` 负责 ICC 回写。处理会应用 EXIF 方向，保留 ICC 与 PNG 显示信息，并移除 EXIF、GPS、XMP 和文本隐私元数据。
