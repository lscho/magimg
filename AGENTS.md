...
- AI 抠图的独立框沿用 SAM -> ViTMatte 透明素材链路；95% 几何包含且面积比低于 80% 的嵌套框自动建立最近父子关系，叶子输出前景、有子级的父框输出修复背景。背景移除合并子级 ViTMatte Alpha 与附近 SAM 弱响应，并按子元素长边 2.5% 动态扩张（4–18 px），减少阴影、描边和碎片残留。选框上不显示编号或用途文字，删除按钮仅在悬停或键盘聚焦时出现；框选工具下拖动边框可移动选区，松手重算关系并记录一条撤销历史。消除修复工具支持 SAM 点提示智能吸附以及手动添加/恢复。本地修复自动按素材上下文分流：纯色或缓渐变 UI 使用确定性二维调和扩散；没有直接子框且所有添加笔画都关闭智能吸附时强制使用扩散，与 `npm run test:repair` 一致；嵌套子框、智能吸附或复杂纹理可使用 `Carve/LaMa-ONNX` 提交 `c3c0c9e468934d62e79c329e35d82dd09ff8c444` 的 512×512 FP32 模型。模型单独按需下载并由 `cutout-repair-manifest.json` 管理。本地流程以上一步得到的父级精修 Alpha 收紧素材内容边界，Alpha 外像素和模型方形留白使用素材主背景色，矩形选框余量中的底图不会进入修复上下文，进入修复前释放 SAM 与 ViTMatte 会话。云端档仅在能力字段明确启用时显示，首次上传原图、背景选框坐标和联合蒙版，服务端按每个选框裁剪上下文后分别修复；同一原图微调时复用服务端素材 ID 并只上传新蒙版与选框坐标，该 ID 随本地抠图历史恢复。两档均只合成修复蒙版内 RGB，浏览器预览不能运行或模拟修复。
