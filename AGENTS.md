...
- 基于 BiRefNet Swin-T（/cutout 分割档）、SAM 2.1 Hiera Base+（/auto-layer 保留）、ViTMatte、Big-LaMa 与 Tauri/Rust 原生 ONNX Runtime 的 AI 抠图，支持嵌套框分层、消除修复和透明 PNG 导出。
...
- 自动分层路由 `/auto-layer` 位于第一组 AI 抠图之后。原图默认占满工作区，云修复成功前不显示结果预览；完成后结果抽屉自动展开，桌面默认 50/50 且分隔线可在 25%–75% 拖动，900px 以下结果全宽。元素框以选框为基础外扩长边 8%（8–32px）的提示框提示 SAM，补充贴边元素上下文；素材导出与选区覆盖仍按选框边界。复用一次 encoder，候选默认取最高分，轮廓近似一致时回退到内部更完整者，经 ViTMatte 精修后再用多候选共识恢复封闭内部孔洞；每个框独立提取，不建立自动嵌套关系，素材按选框边界直接导出（不按 Alpha 内容扩张）。文字框跳过 SAM，由本地 PP-OCRv5 按行识别并用整框单行结果复核碎片检测。整页背景先把每个素材/文字框区域按高召回蒙版（元素）或字形蒙版（文字，未识别框用整框矩形）从整图中链式本地修复；蒙版邻域为纯色/缓渐变或低纹理且颜色集中时直接本地提取背景并退还预扣，不创建云端任务；其余情况生成仅含整页背景的云端修复图集，通过独立 `/auto-layer-tasks` 任务调用一次云端图片编辑，成功后覆盖整页背景；修复模型未安装时本地链式修复回退为确定性扩散。OCR/命名资源没有独立下载按钮，首次点击“一键分层”确认下载并在主按钮显示进度；计费使用 `POST /matting mode=autoLayer` 和服务端 `autoLayerCost`（默认 20）。云任务失败或取消由服务端退款并保留不可预览草稿。分层文档只在页面会话中存活；桌面端选区记录最多 50 条，浏览器不能保存或恢复选区、运行模型、模拟推理或导出项目目录。
- 自动分层质量补充：精修 Alpha 确实越过紧选框边缘时，素材导出范围才向对应方向小幅扩展；普通中文 UI 与描边小字自动回退为字重 400，极强粗体证据才使用 600，自动识别不输出 700。父层本地修复仅在直接子层含文字时强制确定性扩散，其余按内容自动分流（纯色/缓渐变走扩散，复杂纹理走 Big-LaMa），修复模型未安装时统一回退扩散。云端上游输入必须清空图集联合蒙版内的 Alpha 与隐藏 RGB，服务端只允许在各分区的原蒙版内合成一次；客户端拆分结果时直接采用服务端 RGB 并保留素材 Alpha，不得再次按软蒙版混合。
...
- AI 抠图推理为纯客户端本地能力，使用 Rust `ort 2.0.0-rc.10` 与原生 ONNX Runtime 1.22 运行 BiRefNet Swin-T（`/cutout` 分割档，单模型直出 mask）与全精度 `Xenova/vitmatte-base-composition-1k` 精修；`/auto-layer` 仍使用量化 SAM 2.1 Hiera Base+ 与 ViTMatte。不再使用 `onnxruntime-web`/WASM；但计费走正式 API：本地抠图开始前调用 `POST /matting`（带 `Idempotency-Key`）预扣 `mattingCost` 积分拿到 `mattingId`，抠图失败或取消时用 `mattingId` 调 `POST /matting/:id/refund` 全额退回，成功则扣费生效。`mattingCost` 由 `GET /capabilities` 下发（默认 5，取值 1–1,000,000 正整数），服务端权威读取不可篡改。未登录时点击抠图弹出 `LoginModal context="matting"`，登录成功后自动重试；积分不足（余额 < `mattingCost` 或预扣返回 409）时禁用按钮并提示充值。右侧不展示模型列表或开关；仅在分割模型与 ViTMatte 任一缺失时显示统一资源包下载提示，两部分均就绪后提示隐藏。一次安装会跳过已就绪部分，并以连续总进度补齐其余资源。`/cutout` 分割档固定 BiRefNet Swin-T（`birefnet-swin-tiny-general.onnx`，224005088 字节，SHA-256 `5600024376f572a557870a5eb0afb1e5961636bef4e1e22132025467d0f03333`，下载源 rembg 官方 GitHub Release），输入 1x3x1024x1024 NCHW（ImageNet 归一化），输出 1x1x1024x1024 logits（sigmoid 转 alpha）；每选区把 bbox 外扩上下文后裁剪缩放到 1024² 单次前向，不编码复用，多选区线性叠加推理时间。SAM 2.1 ONNX 固定到 `onnx-community/sam2.1-hiera-base-plus-ONNX` 提交 `bab18593f44e652f04cf18b60b3690f60e8996b0`，逐文件流式写入 encoder、decoder 与两份 external-data 权重，并按大小和 SHA-256 校验，安装状态写入 `model-manifest.json`；ViTMatte Base 固定到提交 `1290b014b994e95ca1b9dd9c5f72c3b6d5b7236a`（387371620 字节），由 `cutoutRefinerManager.ts` 按 SHA-256 校验并通过 v2 `cutout-refiner-manifest.json` 管理，安装成功后清理旧 Small 文件。模型统一保存到 `appDataDir/models/`，不随安装包分发；Windows NSIS 会把目标架构的 MSVC v143 app-local CRT DLL 放到主程序同目录，以满足官方 ONNX Runtime 的原生依赖。Rust 只从该目录的白名单加载文件并在会话创建前再次校验；前端按 ImageNet 均值方差生成 1024x1024 NCHW 输入并通过 Raw IPC 发送，Rust 持有分割、精修与优化模型会话。取消会终止当前 ONNX Runtime 运行。`/cutout` 矩形框/多边形链路为 BiRefNet -> 三值 trimap -> ViTMatte：BiRefNet 单次前向输出软 alpha，多边形只提供更贴合物体的外接框裁剪提示、不做硬裁剪；智能笔刷降级为纯几何 add/restore 笔画（BiRefNet 无点提示能力）。浏览器预览不能下载与加载模型。官方最新 SAM 3.1 需要 PyTorch/CUDA、授权权重且暂无适配当前链路的官方 ONNX，不能描述为客户端已支持。旧 ViT-H/ViT-L/ViT-B/MobileSAM 文件不会自动删除，但不再展示、下载或加载。框选坐标统一存图像坐标系，结果按选区 bbox 裁剪导出透明 PNG。路由 `/cutout` 在主导航中位于图生图下方，也可从文生图/图生图结果区与图片编辑页右键菜单进入。
