 ...
 */
export function mergeConnectedBackgroundBoxes(boxes: readonly CutoutSelectionBox[]) {
  ...
  for (const source of boxes) {
    ...
    while (changed) {
      ...
      for (let index = clusters.length - 1; index >= 0; index -= 1) {
        ...
        merged = unionBoxes(merged, clusters[index]);
...
export function createAutoLayerBackgroundBoxes(
  ...
  return mergeConnectedBackgroundBoxes(
    keepLargestOverlappingBoxes(boxes).map(box => ({
