...
import ImageEditorImageContextMenu from "./ImageEditorImageContextMenu.vue";
...
const stackStyle = computed(() => ({
  ...
  transform: `translate(calc(-50% + ${editor.panX.value}px), calc(-50% + ${editor.panY.value}px))`
}));
...
onMounted(async () => {
  ...
  await nextTick();
  if (isDialog.value) shell.value?.focus();
...
onBeforeUnmount(() => {
  if (isDialog.value) document.body.style.overflow = previousBodyOverflow;
});
...
function handleKeydown(event: KeyboardEvent) {
  ...
  if (editor.imageContextMenu.value && event.key === "Escape") {
    event.preventDefault();
    closeImageContextMenu(true);
    return;
  }

  if (event.key === "Escape" && isDialog.value) {
    event.preventDefault();
    requestClose();
    return;
  }
  if (event.key === "Escape" && editor.activeTool.value === "crop") {
    event.preventDefault();
    editor.cancelCrop();
    return;
  }
  if (event.key === "Tab" && isDialog.value) trapFocus(event, shell.value);
  else editor.handleKeydown(event);
}

function handlePointerDown(event: PointerEvent) {
  const target = event.target as HTMLElement;
  if (editor.textContextMenu.value && !target.closest("[data-image-editor-text-menu]")) {
    editor.closeTextContextMenu();
  }
  if (editor.imageContextMenu.value && !target.closest("[data-image-editor-image-menu]")) {
    editor.closeImageContextMenu();
  }
}

function handleCanvasPointerDown(event: PointerEvent) {
  if (event.button !== 0 || !editor.startPan(event.clientX, event.clientY)) return;
  event.preventDefault();
...
function handleCanvasPointerMove(event: PointerEvent) {
  if (!editor.panning.value) return;
  event.preventDefault();
  editor.movePan(event.clientX, event.clientY);
}
...
function handleCanvasContextMenu(event: MouseEvent) {
  ...
  const target = event.target as HTMLElement;
  if (
    !target.closest(".editor-canvas-stack") ||
    !props.imageActionsEnabled ||
    editor.textContextMenu.value
  ) {
    editor.closeImageContextMenu();
    return;
  }
...
function closeImageContextMenu(focusCanvas = false) {
  editor.closeImageContextMenu();
  if (focusCanvas) nextTick(() => viewport.value?.focus());
}
...
async function applyEdits() {
  ...
  try {
    ...
    emit("apply", result);
    if (!isDialog.value) editor.markApplied();
  } catch {
    // The editor exposes the user-facing export error beside the apply action.
  }
}
...
<template>
  <section
    ...
    :aria-labelledby="isDialog ? 'image-editor-title' : undefined"
    :aria-label="isDialog ? undefined : '图片编辑工作台'"
    :aria-busy="editor.busy.value"
    :tabindex="isDialog ? -1 : undefined"
    @keydown="handleKeydown"
    @pointerdown.capture="handlePointerDown"
  >
    <header v-if="isDialog" class="image-editor-header">
    ...
    <div class="image-editor-workspace">
      <div class="editor-content-area">
        ...
        <main
          ...
          :class="{
            'is-pan-tool': editor.activeTool.value === 'pan',
            'is-panning': editor.panning.value
          }"
          aria-label="图片编辑画布"
          tabindex="0"
          @contextmenu="handleCanvasContextMenu"
          @lostpointercapture="editor.endPan"
      ...
      <aside class="editor-side-panel" aria-label="编辑设置">
        <div class="editor-side-scroll">
          <ImageEditorInspector
            ...
            :busy="editor.busy.value"
            :crop-height="editor.cropHeight.value"
            :crop-ratio="editor.cropRatio.value"
            :crop-width="editor.cropWidth.value"
            :output-height="editor.outputHeight.value"
            :output-label="editor.outputLabel.value"
            :output-width="editor.outputWidth.value"
            ...
            :zoom-percent="editor.zoomPercent.value"
    ...
    <footer v-if="isDialog" class="image-editor-footer">
      <p v-if="editor.error.value" role="alert">{{ editor.error.value }}</p>
    ...
    <div v-if="isDialog && showDiscardConfirm" class="discard-overlay">
      <section
        ref="discardDialog"
...
.is-page .image-editor-workspace {
