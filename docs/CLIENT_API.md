...
服务端校验能力开关、可用 AI 节点、文件签名、实际尺寸、字节/像素上限、框坐标、预扣归属、档位、退款状态及唯一绑定关系。桌面客户端对达到 2 MiB（服务端上限更小时为上限 75%）的整页图并行生成质量 88、原尺寸 WebP 传输副本，仅在体积变小时采用；该副本只用于上传和云端上下文，不替换本地原图。顶级框先按较小框 65% 包含率去重；阴影外扩后沿任一主轴至少 50% 连续相交的框合并为同一云修复区，但合并后的单区不得超过整页面积 22%（原始单框已超过时不拆分），避免传递式连接形成缺少背景上下文的近整页删除区，服务端再次执行相同处理。服务端从整页图为每个合并框裁出额外上下文，按统一比例排列为深色间隔图集，并在实际回填区外侧绘制洋红定位线；贴原图边缘的裁片用镜像场景补足四周上下文，删除区不得直接接触图集间隔。上游表单只包含这张图集，不包含 `mask`，也不依赖文本像素坐标定位。图集控制在 3,686,400 像素可靠边界内；若不满足 16px 步进、最小像素或 3:1 约束，只添加深色 padding。提示词要求彻底删除定位线内人物、文字、数字、图标、面板、导航、描边、完整阴影和残片，在每个裁片内形成一个连续背景场，并先对齐从四边进入回填区的山体、云、水流、建筑、路径、线条、渐变、纹理、光照和透视；禁止矩形子块、轮廓平移、模糊、涂抹、重复纹理、接缝、空白渐变、雾或半透明遮罩。响应必须精确匹配请求画布；定位线不占采样区域，服务端按完整回填区映射回原尺寸，不做内缩拉伸。客户端最终再次以未压缩原图为基准，仅在去重后的用户原始顶级框选联合区域内向内羽化采纳返回 RGB；提交给服务端的外扩/合并框只用于生成上下文，合并框间未选择区域及框外像素逐像素保留。每个任务最多调用一次 `/v1/images/edits`；旧数据库中的蒙版图集任务仍可由 worker 兼容执行。
