...
2. 若背景或图生图强度需要由用户控制，需要扩展 `POST /tasks` 请求字段并明确校验规则。
3. 任务查询目前采用 2 秒轮询并持续到终态，临时查询失败不会清除任务。若任务量或耗时继续增长，建议提供服务端推荐轮询间隔或事件推送。
4. 结果区已接入复制、本地图片编辑、AI 抠图、桌面“另存为”和打开默认保存位置；当前模式存在成功结果时，生成主按钮显示“重新生成”。编辑入口会把当前结果一次性交接到独立 `/editor` 页面，编辑结果只在该页面会话内生效，不回写生成预览、不上传服务端或写入任务历史。
